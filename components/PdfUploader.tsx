...
export const PdfUploader: React.FC = () => {
  ...
  const decreaseThumbnailSize = useCallback(() => setThumbnailSize(prev => Math.max(prev - 1, 1)), [])
  ...
  const getSectionColor = (pageNumber: number) => {
    const sectionIndex = [1, ...splitPoints, thumbnails.length + 1].findIndex(sp => pageNumber < sp) - 1
    return colors[sectionIndex % colors.length]
  }

  const getPartName = (pageNumber: number) => {
    const allSplitPoints = [1, ...splitPoints, thumbnails.length + 1].sort((a, b) => a - b)
  ...
  const getPartStatus = useMemo(() => {
    ...
    return allSplitPoints.slice(0, -1).map((startPage, index) => {
      ...
      const deletions = shadedPages.filter(p => p >= startPage && p <= endPage).length
      return {
  ...
  const handleDownloadZip = async () => {
    ...
    if (folder) {
      for (const pdf of splitPDFs) {
        ...
        const partName = partNames[partIndex] ? ` - ${partNames[partIndex]}` : ''
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div 
        className={`flex items-center p-4 border rounded-lg ${isDragging ? 'border-blue-500 bg-blue-50' : 'border-gray-300'}`}
        ...
        {fileInfo ? (
          <div className="flex-grow">
            ...
            <p className="text-sm text-gray-500">
              {fileInfo.size} MB | {fileInfo.pageCount} pages | Last modified: {fileInfo.lastModified}
            </p>
          </div>
        ) : (
          <p className="text-gray-500">Drag and drop a PDF here, or click to select</p>
        )}
      </div>
      {thumbnails.length > 0 && (
        <div className="mt-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex items-center space-x-2">
              <Button onClick={decreaseThumbnailSize} size="sm" variant="outline" title="Zoom Out (or use '-' key)">
                <Minus size={16} />
              </Button>
          ...
          >
            <div className={`grid ${gridCols} gap-4`}>
              {thumbnails.map((thumbnail, index) => {
                ...
                const isStartOfPortion = pageNumber === 1 || splitPoints.includes(pageNumber)
                return (
                    ...
                    <div className={`p-2 ${sectionColor} ${isShaded ? 'bg-gray-400 bg-opacity-50' : ''}`}>
                      ...
                      <div className="flex justify-between items-center mt-1">
                        {isStartOfPortion && (
                          <span 
                            ...
                            onClick={() => {
                              const idx = parseInt(partName.replace('part', ''))
                              const newName = prompt("Enter name for this part:", partNames[idx] || partName.replace('part', 'Part '))
                              if (newName !== null) {
                                handlePartNameChange(idx, newName)
                              }
                            }}
                          >
                            {partNames[parseInt(partName.replace('part', ''))] || partName.replace('part', 'Part ')}
                          </span>
                        )}
                      ...
                      <Button
                        className={`
                          ...
                          ${isShaded ? 'bg-gray-600 text-white' : 'bg-white text-black border border-gray-300'} 
                    ...
                    {index < thumbnails.length - 1 && (
                      <div 
                        className={`absolute top-0 right-0 w-1 h-full cursor-pointer transition-all duration-200 ease-in-out
                                    ${splitPoints.includes(pageNumber + 1) ? 'bg-blue-500 w-2' : 'bg-transparent group-hover:bg-gray-300'}`}
        ...
        <div className="mt-4">
          {splitPoints.length > 0 && (
            <>
              ...
              <p>The PDF will be split before pages: {splitPoints.join(', ')}</p>
            </>
          )}
          {shadedPages.length > 0 && (
            <>
              <h3 className="text-lg font-semibold mb-2">Shaded Pages</h3>
              <p>The following pages will be removed: {shadedPages.join(', ')}</p>
            </>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {getPartStatus.map((status, index) => (
              <div key={index} className="border p-2 rounded">
                <p><strong>{status.name}:</strong> {status.pageCount} Pages 
                  {status.deletions > 0 && ` (${status.deletions} deletion${status.deletions > 1 ? 's' : ''})`}
                </p>
              </div>
            ))}
          </div>
    ...
    {splitPDFs.length > 0 && (
        <div className="mt-4">
          ...
          {splitPDFs.map((pdf, index) => {
            ...
            const partName = partNames[partIndex] ? ` - ${partNames[partIndex]}` : ''
            const displayName = partNames[partIndex] ? `${partNames[partIndex].substring(0, 20)}` : `Part ${partIndex}`
            return (
              <Button 
                key={index} 
                ...
                asChild
                onClick={(e) => {
                  e.currentTarget.classList.remove('bg-gray-200', 'text-black', 'hover:bg-gray-300')
          ...
          <Button 
            ...
            className={`mt-4 ${isZipDownloaded ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black hover:bg-gray-300'}`}
          >
