...
- [~] **Tests** — vitest added (`pnpm test`), **13 tests** covering the core logic: `getPartInfo` (part identity / #4 anchor), `computePartRanges`, and `splitPdfDocument` (real pdf-lib integration). Still uncovered: the hook's async/pdf.js paths (need `renderHook` + pdf.js mocking) and component/UI tests.
- [ ] **CI** — no GitHub Actions yet; min bar = run `pnpm test` + `pnpm build` on push.
